...
class AgentEngineTest {
    ...
    @Test
    fun prepareChatRequest_injectWorkflowStageFalse_omitsCurrentStageBlock() {
        val engineNoTools = AgentEngine(repository, memoryManager, emptyList())
        val prepared = engineNoTools.prepareChatRequest(
            defaultAgent,
            AgentStage.EXECUTION,
            isJsonMode = false,
            injectWorkflowStageIntoPrompt = false,
        )
        assertFalse(prepared.systemPrompt.contains("CURRENT STAGE"), prepared.systemPrompt)
        assertEquals("CHAT", prepared.snapshot.agentStage)
    }
