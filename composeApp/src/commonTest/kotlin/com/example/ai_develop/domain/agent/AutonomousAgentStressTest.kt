...
class AutonomousAgentStressTest {
    ...
    @Test
    fun ordinary_sendMessage_setsTaskIdAndTaskState() = runTest {
        repository.agentStateMap[agentId] = AgentState(
            agentId = agentId,
            workflowStagesEnabled = true,
        ...
        val agent = AutonomousAgent(
            ...
            fakeEngine,
            backgroundScope,
            taskIdForMessagePersistence = "task-1",
        )
        advanceUntilIdle()

        fakeEngine.queueResponse(listOf("done"))
        agent.sendMessage("hello").collect()
        advanceUntilIdle()

        val user = agent.uiState.value.agent?.messages?.firstOrNull { it.role == "user" }
        assertNotNull(user)
        assertEquals("task-1", user.taskId)
        assertEquals(TaskState.EXECUTION, user.taskState)
    ...
    @Test
    fun lifecycle_corner_doubleDispose() = runTest {
        val agent = AutonomousAgent(agentId, repository, fakeEngine, backgroundScope)
        advanceUntilIdle()
        agent.dispose()
        agent.dispose()
    ...
    @Test
    fun lifecycle_corner_disposeThenSendMessage_returnsImmediately() = runTest {
        val agent = AutonomousAgent(agentId, repository, fakeEngine, backgroundScope)
        advanceUntilIdle()
        fakeEngine.queueResponse(listOf("ok"))
        agent.dispose()

        val out = agent.sendMessage("hi").toList()
        assertTrue(out.isEmpty(), "Disposed agent must not run LLM pipeline")
        assertFalse(agent.uiState.value.isProcessing)
    }

    // --- 2. Corner cases ---

    @Test
    fun corner_sendWelcomeMessage_emptyHistory() = runTest {
        repository.agentStateMap[agentId] = AgentState(
            agentId = agentId,
            name = "Stress Agent",
            messages = emptyList(),
        ...
        fakeEngine.queueResponse(listOf("Welcome!"))
        val agent = AutonomousAgent(agentId, repository, fakeEngine, backgroundScope)
        advanceUntilIdle()
        ...
        assertFalse(agent.uiState.value.isProcessing)
        val assistants = agent.uiState.value.agent?.messages?.filter { it.role == "assistant" }.orEmpty()
        assertEquals(1, assistants.size)
        assertTrue(assistants.single().message.contains("Welcome"), assistants.single().message)
        assertTrue(
            repository.agentStateMap[agentId]?.messages?.any { it.role == "assistant" } == true,
            "assistant row should be persisted",
        )
    ...
    @Test
    fun corner_sendWelcomeMessage_skipsWhenHistoryNonEmpty() = runTest {
        repository.agentStateMap[agentId] = AgentState(
            agentId = agentId,
            messages = listOf(
                ChatMessage(
        ...
        val engine = FakeAgentEngine(repository, memoryManager)
        val agent = AutonomousAgent(agentId, repository, engine, backgroundScope)
        advanceUntilIdle()

        agent.sendWelcomeMessage().collect()
        advanceUntilIdle()

        assertEquals(0, engine.streamFromPreparedCallCount)
        assertEquals(1, agent.uiState.value.agent?.messages?.size)

        agent.dispose()
    }

    @Test
    fun corner_runtimeSnapshot_prefersPersistedRagEnabled() = runTest {
        repository.agentStateMap[agentId] = AgentState(
            agentId = agentId,
            ragEnabled = false,
            workingMemory = WorkingMemory(),
        ...
        repository.agentStateMap[agentId] = repository.agentStateMap[agentId]!!.copy(ragEnabled = true)
        engine.queueResponse(listOf("ack"))
        var captured: Boolean? = null
        engine.onStreamFromPreparedAgent = { captured = it.ragEnabled }
        agent.sendMessage("ping").collect()
        advanceUntilIdle()

        assertEquals(true, captured, "persisted ragEnabled=true must override in-memory false for LLM step")

        agent.dispose()
    }

    @Test
    fun corner_transitionInvalid_doesNotSyncRepository() = runTest {
        repository.agentStateMap[agentId] = AgentState(
            agentId = agentId,
            workflowStagesEnabled = true,
        ...
        val result = agent.transitionTo(AgentStage.DONE)
        assertTrue(result.isFailure)
        assertEquals(AgentStage.PLANNING, repository.agentStateMap[agentId]?.currentStage)

        agent.dispose()
    }

    @Test
    fun corner_performMaintenance_updatesUiAndRepository() = runTest {
        class MaintEngine(
            repo: ChatRepository,
            mem: ChatMemoryManager,
        ) : FakeAgentEngine(repo, mem) {
            override suspend fun performMaintenance(agent: Agent): WorkingMemory =
                agent.workingMemory.copy(currentTask = "after")
        }

        val engine = MaintEngine(repository, memoryManager)
        val agent = AutonomousAgent(agentId, repository, engine, backgroundScope)
        advanceUntilIdle()

        engine.queueResponse(listOf("hi"))
        agent.sendMessage("x").collect()
        advanceUntilIdle()

        assertEquals("after", agent.uiState.value.agent?.workingMemory?.currentTask)
        assertEquals("after", repository.agentStateMap[agentId]?.workingMemory?.currentTask)
    ...
    @Test
    fun stress_observeEmitsStaleMessages_whileProcessing_prefersLocalDraft() = runTest {
        fakeEngine.queueResponse((1..12).map { "chunk$it;" })
        fakeEngine.afterFirstStreamChunk = suspend {
            val base = repository.agentStateMap[agentId]
            assertNotNull(base)
            val userOnly = listOfNotNull(base.messages.firstOrNull { it.role == "user" })
            assertTrue(userOnly.isNotEmpty(), "expected user message in repository snapshot")
            repository.emitObservedState(base.copy(messages = userOnly))
        }
        val agent = AutonomousAgent(agentId, repository, fakeEngine, backgroundScope)
        advanceUntilIdle()
        ...
        assertNotNull(assistant)
        assertTrue(
            assistant.message.contains("chunk12"),
            "local streaming draft must survive stale observe while processing: ${assistant.message}",
        )
    ...
    @Test
    fun stress_toolChainRespectsMaxIterations() = runTest {
        var toolRuns = 0
        val stepTool = object : AgentTool {
            override val name = "step_tool"
            override val description = "stub"
            override suspend fun execute(input: String): String {
                toolRuns++
                return "ok($input)"
        ...
        val engine = FakeAgentEngine(repository, memoryManager, listOf(stepTool))
        val sixTools = (1..6).joinToString(" ") { n -> "[TOOL: step_tool($n)]" }
        engine.queueResponse(listOf(sixTools))
        ...
        val agent = AutonomousAgent(
            ...
            engine,
            backgroundScope,
            maxToolChainIterations = 4,
        )
        advanceUntilIdle()

        agent.sendMessage("run").collect()
        advanceUntilIdle()

        assertEquals(
            4,
            toolRuns,
            "same-response tool chain must stop after maxToolChainIterations",
        )

        agent.dispose()
    }

    open class FakeAgentEngine(
        repo: ChatRepository,
        mem: ChatMemoryManager,
        ...
        /** Вызывается перед стримом; для проверки [AutonomousAgent.resolveRuntimeSnapshot]. */
        var onStreamFromPreparedAgent: ((Agent) -> Unit)? = null

        /** После первого эмита чанка (пока [isProcessing] ещё true) — для гонки с observe. */
        var afterFirstStreamChunk: (suspend () -> Unit)? = null

        fun queueResponse(chunks: List<String>) = responses.add(chunks)
        fun queueToolResult(res: String?) = toolResults.add(res)

        override fun streamFromPrepared(agent: Agent, prepared: PreparedLlmRequest): Flow<String> = flow {
            onStreamFromPreparedAgent?.invoke(agent)
    ...
    class MockChatRepository : ChatRepository {
        ...
        private val _agentUpdates = MutableSharedFlow<AgentState>(
            replay = 1,
        ...
        /** Имитация отстающего observe без изменения [agentStateMap]. */
        suspend fun emitObservedState(state: AgentState) {
            _agentUpdates.emit(state)
        }
